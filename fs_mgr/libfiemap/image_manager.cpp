...
#include "utility.h"
    ...
    bool ImageManager::DeleteBackingImage(const std::string& name) {
        ...
        if (IsImageMapped(name)) {
            LOG(ERROR) << "Cannot delete backing image " << name
                       << " because mapped to a block device";
            return false;
        }
    ...
    bool ImageManager::MapImageDevice(const std::string& name,
        ...
        if (!MapWithDmLinear(*partition_opener_.get(), name, timeout_ms, path)) {
            LOG(WARNING)
                    << "MapWithDmLinear failed, falling back to MapWithLoopDevice for DFE users.";
    ...
    bool ImageManager::MapImageWithDeviceMapper(const IPartitionOpener& opener,
                                                const std::string& name, std::string* dev) {
        std::string ignore_path;
        if (!MapWithDmLinear(opener, name, {}, &ignore_path)) {
            LOG(WARNING)
                    << "MapWithDmLinear failed, falling back to MapWithLoopDevice for DFE users.";
            if (!MapWithLoopDevice(name, std::chrono::milliseconds(0), &ignore_path)) {
                return false;
            }
            *dev = ignore_path;  // ignore_path holds the loop device path
            // Set a property so we remember this is mapped.
            auto prop_name = GetStatusPropertyName(name);
            if (!android::base::SetProperty(prop_name, ignore_path)) {
                UnmapImageDevice(name, true);
                return false;
            }
            return true;
    ...
    static bool CompareExtents(android::fs_mgr::Partition* oldp, android::fs_mgr::Partition* newp) {
        ...
        while (true) {
            ...
            if (!CompareExtent(old_iter->get(), new_iter->get())) {
                LOG(ERROR) << "Extents do not match: " << old_iter->get() << ", "
                           << new_iter->get();
                return false;
            }
    ...
    MappedDevice::MappedDevice(IImageManager* manager, const std::string& name,
    ...
    bool IImageManager::UnmapImageIfExists(const std::string& name) {
        ...
        // happens after process B checks IsImageMapped(), it would be as if MapImageDevice() is
        // called after process B finishes calling UnmapImageIfExists(), resulting the image to be
        // mapped, which is a reasonable sequence.
        if (!IsImageMapped(name)) {
            return true;
        }
